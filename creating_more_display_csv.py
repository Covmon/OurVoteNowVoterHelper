...
def gender_vs_race_statewide(year,election_type,df_voted,df_registered,primary):
	...
	df_registered = df_registered.loc[df_registered['COUNTY NAME']=='TOTAL']
...
def gender_vs_race_precinct(year,election_type,df_voted,df_registered,county,primary):	
	...
	for precinct in df_voted['PRECINCT NAME'].unique():
		...
		for col,value in zip(df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith(' MALE')]].columns,df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith(' MALE')]].iloc[len(df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith(' MALE')]])-1]):
			col = col.replace(' MALE','')
			if col == 'UNKNOWN':
				df_v.loc[len(df_v)] = [col,value,'',0]
			else:	
				df_v.loc[len(df_v)] = [col,value,'','']
		for col,value in zip(df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith(' FEMALE')]].columns,df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith(' FEMALE')]].iloc[len(df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith(' FEMALE')]])-1]):
			col = col.replace(' FEMALE','')
			df_v.loc[df_v['RACE']==col,'FEMALE'] = value
		for col,value in zip(df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith('UNKNOWN')]].columns,df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith('UNKNOWN')]].iloc[len(df_voted_precinct[df_voted_precinct.columns[pd.Series(df_voted_precinct.columns).str.endswith('UNKNOWN')]])-1]):
			if col != 'UNKNOWN':
				col = col.replace(' UNKNOWN','')
			df_v.loc[df_v['RACE']==col,'UNKNOWN'] = value
		df_v.fillna(0,inplace=True)	
		...
		df_r = pd.DataFrame(columns=['RACE','MALE','FEMALE','UNKNOWN'])
		for col,value in zip(df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith(' MALE')]].columns,df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith(' MALE')]].iloc[len(df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith(' MALE')]])-1]):
			col = col.replace(' MALE','')
			if col == 'UNKNOWN':
				df_r.loc[len(df_r)] = [col,value,'',0]
			else:	
				df_r.loc[len(df_r)] = [col,value,'','']
		for col,value in zip(df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith(' FEMALE')]].columns,df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith(' FEMALE')]].iloc[len(df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith(' FEMALE')]])-1]):
			col = col.replace(' FEMALE','')
			df_r.loc[df_r['RACE']==col,'FEMALE'] = value
		for col,value in zip(df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith('UNKNOWN')]].columns,df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith('UNKNOWN')]].iloc[len(df_registered_precinct[df_registered_precinct.columns[pd.Series(df_registered_precinct.columns).str.endswith('UNKNOWN')]])-1]):
			if col != 'UNKNOWN':
				col = col.replace(' UNKNOWN','')
			df_r.loc[df_r['RACE']==col,'UNKNOWN'] = value
...
def race_vs_inter_year_county(county,years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
	
	election_type_dict = {'Presidential_Primary': 'Presidential Primary','Primary': 'Primary','Primary_Runoff': 'Primary Runoff','General': 'General'}
	...
	for year in years.keys():
		if any(years[year]):
			for election_type in years[year]:
				...
				election_type = election_type_dict[election_type]
...
def gender_vs_inter_year_county(county,years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def age_vs_inter_year_county(county,years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
	...
	for year in years.keys():
		if any(years[year]):
			for election_type in years[year]:
				...
				df_combined = df_combined.append(df_year,ignore_index=True)
...
def race_vs_inter_year_statewide(years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def gender_vs_inter_year_statewide(years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def age_vs_inter_year_statewide(years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def race_vs_inter_year_congressional(congressional_district,years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def gender_vs_inter_year_congressional(congressional_district,years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def county_vs_inter_year_statewide(years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def precinct_vs_inter_year(county,years={'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def congressional_vs_inter_year_statewide(years={'2014':['Primary','Primary_Runoff','General'],'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
...
def race_vs_inter_year_precinct(county,precinct,years={'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
		...
		if any(years[year]):
			for election_type in years[year]:
				try:
					...
					df_year = pd.DataFrame.from_dict({'ELECTION TYPE/YEAR':['{} {}'.format(year,election_type)],'ASIA-PI':['-'],'BLACK':['-'],'HISP-LT':['-'],'NATIVE-AM':['-'],'OTHER':['-'],'WHITE':['-'],'UNKNOWN':['-'],'TOTAL':['-']})
				else:
					df_year = df_year[['RACE','TOTAL']].T
					df_year.columns = df_year.iloc[0]
					df_year = df_year.reindex(df_year.index.drop('RACE'))
...
def gender_vs_inter_year_precinct(county,precinct,years={'2016':['Presidential_Primary','Primary','Primary_Runoff','General'],'2018':['Primary','Primary_Runoff','General']}):
		...
		if any(years[year]):
			for election_type in years[year]:
				try:
					...
					df_year = pd.DataFrame.from_dict({'ELECTION TYPE/YEAR':['{} {}'.format(year,election_type)],'MALE':['-'],'FEMALE':['-'],'UNKNOWN':['-'],'TOTAL':['-']})
					df_combined = df_combined.append(df_year,ignore_index=True)
				else:
					df_combined = df_combined.append(df_year.loc[df_year['RACE']=='TOTAL'],ignore_index=True)
				finally:
